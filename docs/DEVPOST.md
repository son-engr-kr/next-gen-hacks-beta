...
**1. Find candidates by saying what you actually want.** Press the mic and speak in plain English or Korean: *"wheelchair accessible Korean restaurant with parking, under forty dollars per person."* Gemini extracts structured filters (cuisine, price, vibe, **wheelchair access, parking, dogs, alcohol**) and a 3D Boston map filters in real time — non-matching buildings sink into the ground and vanish, matching ones get a glowing spotlight. Cards slide in on the right. A friendly voice speaks back the count and top picks.

**2. Call the candidates and ask your questions, automatically and in parallel.** Voice search filters by *static* attributes Google Places knows about (wheelchair, parking, hours). The phone agent fills in the *dynamic* attributes that only a staff member knows: current wait time, today's vegetarian dishes, whether they have a high chair right now. Pick from the question checklist (reservations, vegetarian, dogs, live music, outdoor seating…), add a custom question in any language (e.g. Korean: *유아 의자 있어요?* — "do you have a high chair?"), click "Call N selected." Our AI agent dials each restaurant, asks **one question at a time** in a chained Twilio Gather flow, transcribes with Google Cloud Speech, parses with Gemini, and streams the answers back as a structured ✓/✗/? checklist per restaurant.
